...
= 斬新さの主張
RubyのGCにはこれまで大きな改善は行われきませんでした．その理由として，
GCは言語処理系と密に連携しており，大規模な修正が必要なる場合が多いとい
う点が上げられます．世代別GCやインクリメンタルGCを導入したい場合はライ
トバリアが必要になりますし，コピーGCを導入したい場合はexactGCにする
為，Cの拡張ライブラリを全て書き直さなくてはなりません．
しかし，今回提案する一部世代別GC化はより少ない修正で，かつ実行速度の大
きな改善が見込めます．その点で中々斬新なのではないかと考えています．

= 期待される効果
期待される効果としては，「RubyGCの実行速度が速くなる」という点しかあり
ません．それだけか？と言わればそうなのですが，これ程，明確な効果もない
のではないかと思います．RubyGCの実行速度が0.1秒でも高速化されれば，世界
中のRubyユーザの0.1秒を無駄に使わずに済むわけですから，これはとても名誉
な事だと考えています．

= 具体的な進め方と予算

== 主に開発を行う場所
自宅，もしくは職場（ネットワーク応用通信研究所）を想定しています．

== 使用する計算機環境(ハード、OS)
* OS : Ubuntu 8.04(linux 2.6.24-23-generic)
* PC : ThinkpadT60
...
いつまでになにをやるか、手順と時期(月単位) を明確に書いたもの

== 開発にかかわる時間帯と時間数
（TODO：時間帯時間数）
毎週どのような時間帯に開発を行うかをおおまかに書いてください。学生、組
織人の場合、フルタイムの開発は難しいでしょうから。また、開発フェーズで
変動する場合は、簡単にリマークしておいてください。たとえば、7 月は本業
や試験で開店休業だが、8 月は普段の2 倍集中するなど。
...
== 予算内訳をまとめた表
（TODO：表）
プロジェクト管理組織の費用として、一律50 万円を組み込むことを忘れないで
ください。その残りの予算の多くの部分を開発者の人件費にしても構いません。
未踏ユースでは開発者1 名の人件費は1 時間あたり一律2,000 円です。この基
準に合わせて積算をしてください。そのほか、PC 等の購入についてはいろいろ
な縛りがあります。予算の詳細は、採択されてからの契約時にプロジェクト管
理組織やPM と相談して調整することになります。
...
= ソフトウェア以外の趣味など
サッカーでしょうか．フットサルチームを友達と作って試合や練習をしていました．
島根に来てからはスノーボードをやっています．冬になると会社の先輩と雪山
に向かって，ジャンプやトリックを練習しています．
